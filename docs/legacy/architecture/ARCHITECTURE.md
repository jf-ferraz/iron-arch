...
The TUI layer maintains additional state for user interactions and operations:

```rust
/// Pre-flight check results before system update
pub struct PreflightResult {
    /// Network connectivity status
    pub network_ok: bool,
    /// Disk space check (requires ~500MB free)
    pub disk_ok: bool,
    /// Battery status (>20% or plugged in)
    pub battery_ok: bool,
    /// Pacman lock file status
    pub pacman_unlocked: bool,
    /// System time synchronization status
    pub time_synced: bool,
    /// Unacknowledged Arch news items
    pub news: Vec<UnacknowledgedNews>,
}

/// Post-update detection results
pub struct PostUpdateResult {
    /// Detected .pacnew/.pacsave config conflicts
    pub config_conflicts: Vec<ConfigConflict>,
    /// Failed systemd services
    pub failed_services: Vec<String>,
    /// Whether reboot is recommended (kernel update, etc.)
    pub reboot_recommended: bool,
}

/// Cleanup categories (8 total: 6 safe, 2 aggressive)
pub enum CleanupCategory {
    // Safe categories (dry-run not required)
    PackageCache,           // Old package versions
    OrphanPackages,         // Unused dependencies
    TempFiles,              // /tmp, /var/tmp
    JournalLogs,            // systemd journal (>7 days)
    UserCache,              // ~/.cache
    BuildArtifacts,         // ~/.cargo/registry, etc.

    // Aggressive categories (dry-run recommended)
    OldKernels,             // Previous kernel versions
    UserTrash,              // Trash directories
}
...
│  │   ├── ActionCard (Update, Cleanup, Doctor)                               │
│  │   └── MaintenanceTimestamps                                              │
│  │                                                                          │
│  ├── CleanSystem                                                            │
│  │   ├── CategorySelector (8 categories: 6 safe, 2 aggressive)             │
│  │   ├── CleanupPreviewList                                                 │
│  │   └── CleanupResultsSummary                                              │
│  │                                                                          │
│  ├── ConfigManager (.pacnew/.pacsave)                                       │
│  │   ├── ConflictList                                                       │
│  │   └── DiffViewer                                                         │
│  │                                                                          │
│  ├── SecurityModules                                                        │
│  │   ├── SecurityModuleList (filtered by security-related IDs)             │
│  │   └── ModuleToggle                                                       │
│  │                                                                          │
│  ├── OperationLog                                                           │
│  │   ├── OperationList (from .iron/logs/operations.jsonl)                  │
│  │   └── OperationFilter                                                    │
│  │                                                                          │
│  ├── Settings                                                               │
│  │   ├── HostSettingsWidget                                                 │
...
The TUI uses expiring status messages for user feedback:

```rust
/// Status message with auto-expiration
pub struct StatusMessage {
    pub text: String,
    pub level: MessageLevel,
    pub created_at: Instant,
    pub duration: Duration,  // Default: 3s, Extended: 5s
}
...
pub enum MessageLevel {
    ...
    Error,    // Red, error prefix (uses error_message slot)
}
```
...
pub enum Dialog {
    Help(HelpContent),              // View-specific keybindings
    Confirm(ConfirmAction),         // Yes/No confirmation
    Progress(ProgressDialog),       // Long-operation spinner
    Error(String),                  // Error display
    Info(String),                   // Info display
}

/// Confirm action callback pattern
...
pub struct ProgressTracker {
    ...
    pub percentage: Option<u8>,  // None = indeterminate (spinner)
    pub complete: bool,
    pub failed: bool,
    started_at: Instant,
}
...
Each view provides context-sensitive keybindings via `get_view_keybindings(view)`:

| View | Specific Keys |
|------|---------------|
| Dashboard | `[u]` Update, `[c]` Cleanup, `[b]` Bundles |
| SystemMaintenance | `[u]` Update, `[c]` Cleanup, `[d]` Doctor |
| CleanSystem | `[Space]` Toggle, `[a]` Toggle All, `[p]` Preview |
| ConfigManager | `[Enter]` View Diff, `[r]` Mark Resolved |
| SecurityModules | `[Enter]` Toggle, `[i]` Install |
| OperationLog | `[f]` Filter, `[/]` Search |
...
│  │                                                                      │    │
│  │  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐           │    │
│  │  │  Cleanup     │    │  Pre-flight  │    │ Post-update  │           │    │
│  │  │  Service     │    │  Checks      │    │ Detection    │           │    │
│  │  │              │    │              │    │              │           │    │
│  │  │ • 8 cats     │    │ • Network    │    │ • .pacnew    │           │    │
│  │  │ • Preview    │    │ • Disk       │    │ • .pacsave   │           │    │
│  │  │ • Execute    │    │ • Battery    │    │ • Services   │           │    │
│  │  │ • Dry-run    │    │ • Pacman     │    │ • Reboot     │           │    │
│  │  │              │    │ • Time sync  │    │              │           │    │
│  │  │              │    │ • News       │    │              │           │    │
│  │  └──────────────┘    └──────────────┘    └──────────────┘           │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
...
|----------|------|-------------|
| PackageCache | Safe | Old package versions in /var/cache/pacman/pkg |
| OrphanPackages | Safe | Unused package dependencies |
| TempFiles | Safe | /tmp and /var/tmp contents |
| JournalLogs | Safe | systemd journal logs >7 days |
| UserCache | Safe | ~/.cache directory |
| BuildArtifacts | Safe | ~/.cargo/registry, build dirs |
| OldKernels | Aggressive | Previous kernel versions |
| UserTrash | Aggressive | Trash directories |
